...
class LSTM(nn.Module):
    def __init__(self, input_size=1, hidden_size=20):
        ...
        # input gate 
        self.W_ii = nn.Linear(self.input_size, self.input_size, bias=True)
        self.W_hi = nn.Linear(self.hidden_size, self.input_size, bias=True)
    ...
    def forward(self, x, h=None, c=None):
        ...
        lengths = None
        if isinstance(x, PackedSequence):
            x, lengths = pad_packed_sequence(x, lengths, batch_first=True)
        ...
        for i in range(seq_len):
          ...
          input_gate = self.sigmoid(self.W_ii(x[i]) + self.W_hi(h))
          forget_gate = self.sigomid(self.W_if(x) + self.W_hf)
          cell_gate = self.tanh(self.W_ig(x) + self.W_hg(h)  )
          output_gate = self.sigmoid( self.W_io(x) + self.W_ho(h) )
          c = forget_gate * c + input_gate * cell_gate 
          h = output_gate * self.tanh(c)
          h_seq.append(h) #save current hidden state
        h_seq = torch.stack(h_seq, dim=0) #convert h_seq from list to tensor 
        
        # reshaping hidden state and cell state
        h.unsqueeze(dim=0)
        c.unsqueeze(dim=0)
